...
Lemma subterm_sn : forall t, SN t -> forall u, subterm_eq u t -> SN u.

Proof.
intros t H. elim H. clear t H. intros t H IH u H0. apply Acc_intro.
intros u' H1. deduce (red_subterm H1 H0). destruct H2 as [t']. destruct H2.
eapply IH. unfold transp. apply H2. assumption.
Qed.

Lemma sub_sn : forall l x s, In x (varlist l) -> SN (app s l) -> SN (s x).

Proof.
intros. change (SN (app s (Var x))). eapply subterm_sn. apply H0.
apply subterm_app. apply in_varlist_subterm. assumption.
Qed.

Lemma sub_fun_sn : forall f ts x s,
  In x (varlist (Fun f ts)) -> Vforall SN (Vmap (app s) ts) -> SN (s x).

Proof.
intros. change (SN (app s (Var x))). rewrite varlist_fun in H.
...
Lemma sn_var : forall v, SN (Var v).

Proof.
intro. apply Acc_intro. intros t H. redtac. absurd (@notvar Sig (Var v)).
...
Lemma sn_args_sn_fun : forall f, defined f R = false ->
  forall ts, Vforall SN ts -> SN (Fun f ts).

Proof.
intros. set (lt := (@Vlt_prod term (transp Red) (arity f))).
assert (Acc lt ts). unfold lt. apply Vforall_Acc_lt_prod. assumption.
elim H1. intros.
apply Acc_intro. change (forall y, Red (Fun f x) y -> SN y). intros.
...
(* C <> Hole *)
Funeqtac. subst x. apply H3. unfold lt. eapply Vlt_prod_cast. apply Vlt_prod_app.
apply Vlt_prod_cons. left. split. 2: reflexivity.
unfold transp. apply red_rule. assumption.
Qed.
...
Lemma no_call_app_sn : forall t, calls R t = nil -> forall s,
  (forall x, In x (varlist t) -> SN (s x)) -> SN (app s t).
...
Lemma calls_sn_args : forall r s, (forall x, In x (varlist r) -> SN (s x))
  -> (forall g vs, In (Fun g vs) (calls R r)
                   -> Vforall SN (Vmap (app s) vs) -> SN (app s (Fun g vs)))
  -> forall g vs, In (Fun g vs) (calls R r) -> Vforall SN (Vmap (app s) vs).

Proof.
intros r s H H0. cut (forall a g vs, a = Fun g vs
  -> In a (calls R r) -> Vforall SN (Vmap (app s) vs)).
...
Lemma calls_sn : forall r s, (forall x, In x (varlist r) -> SN (s x))
  -> (forall g vs, In (Fun g vs) (calls R r)
                   -> Vforall SN (Vmap (app s) vs) -> SN (app s (Fun g vs)))
  -> forall a, In a (calls R r) -> SN (app s a).
