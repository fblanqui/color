...
Fixpoint SCC'_list_aux i L {struct L} := match L with
  | nil => nil
  | x::q => match eq_opt_nat_dec (SCC'_tag hyps HM x) (Some i) with
...
Lemma SCC'_list_aux_exact i L r : In r (SCC'_list_aux i L) <-> In r L /\ (SCC'_tag hyps HM r)=(Some i).
Proof.
intros.
induction L.
simpl in *. tauto.
...
split;intro.
simpl in *.
destruct (eq_opt_nat_dec (SCC'_tag hyps HM a) (Some i));destruct (rule_eq_dec a r);simpl in *;intuition.
subst a;tauto.

destruct H.
simpl in *.
destruct (eq_opt_nat_dec (SCC'_tag hyps HM a) (Some i));destruct H;simpl in *; try subst a;tauto.
Qed.

Lemma repeat_free_SCC'_list_aux i L :repeat_free L -> repeat_free (SCC'_list_aux i L).
Proof.
...
Lemma repeat_free_SCC'_list i :repeat_free (SCC'_list i).
Proof.
...
assert(In h (SCC'_list i));assert(In h0 (SCC'_list i));try rewrite <- H0;simpl;auto.
