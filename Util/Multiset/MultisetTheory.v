...
Section meq_equivalence.
  ...
  Lemma meq_trans : forall M N P, M =mul= N -> N =mul= P -> M =mul= P.
...
Section MultisetLemmas.
  ...
  Lemma intersection_comm : forall M N, M # N =mul= N # M.
  ...
  Lemma member_singleton : forall x y, x in {{y}} -> x =A= y.
  ...
  Proof.
    intros.
    destruct (eqA_dec a b); trivial.
    absurd (mult a {{b}} > 0); trivial.
    rewrite singleton_mult_notin; trivial.
    omega.
  Qed.

  Lemma singleton_mult_union_empty : forall a a' M, 
  ...
  Lemma union_empty : forall M, M + empty =mul= M.

  Proof.
    solve_meq.
  Qed.

  Lemma notempty_member : forall M, M <>mul empty ->
    exists N, exists a, M =mul= {{a}} + N.
  ...
  Lemma not_empty : forall M x, x in M -> M =mul= empty -> False.

  Proof.
    unfold member; intros M x mult_x_M M_is_empty.
    absurd (x/M = 0).
    omega.
  ...
  Lemma member_union_l : forall a M N, a in M -> a in (M + N).
  ...
  Lemma member_union_r : forall a M N, a in N -> a in (M + N).
  ...
  Lemma singleton_member : forall a, a in {{a}}.

  Proof.
    unfold member; intro a.
    assert (a/{{a}} = 1); [idtac | omega].
    apply singleton_mult_in; auto with sets.
  Qed.

  Lemma member_insert : forall a M, a in (insert a M).
  
  Proof.
    unfold insert; intros.
    apply member_union_l.
    apply singleton_member.
  Qed.

  Lemma member_member_union : forall a M N, a in M -> a in (M+N).
  ...
  Lemma member_diff_member : forall a M N, a in (M-N) -> a in M.

  Proof.
    intros a M N; unfold member.
    rewrite (diff_mult M N).
    omega.
  Qed.

  Lemma diff_member_ly_rn : forall a M N,
    a in M -> ~a in N -> a in (M - N).
  ...
  Lemma diff_remove_both : forall a M N,
  ...
  Lemma member_union : forall a M N, a in (M+N) -> a in M \/ a in N.

  Proof.
    intros a M N; unfold member. 
    rewrite (union_mult M N).
    omega.
  Qed.

  Lemma member_meq_union : forall a M N M' N',
  ...
  Lemma meq_union_meq : forall M N P, M + P =mul= N + P -> M =mul= N.
  ...
  Lemma meq_meq_union : forall M N P, M =mul= N -> M + P =mul= N + P.

  Proof.
    solve_meq.
  Qed.

  Lemma meq_ins_ins_eq : forall a a' M M',
  ...
  Lemma meq_ins_ins : forall a a' M M',
  ...
  Lemma member_ins_ins_meq : forall a a' M M',
  ...
  Lemma meq_ins_rem : forall a M, a in M -> M =mul= M - {{a}} + {{a}}.
  ...
  Lemma meq_remove_insert : forall a M, remove a (M + {{a}}) =mul= M.
  
  Proof.
    solve_meq.
  Qed.

  Lemma meq_remove : forall a M N, insert a M =mul= N -> M =mul= remove a N.
  ...
  Lemma insert_meq : forall M N a,
    insert a M =mul= insert a N -> M =mul= N.
  ...
  Lemma insert_remove_eq : forall a a' M,
    a =A= a' -> M =mul= remove a (insert a' M).
  ...
  Lemma meq_insert_remove : forall a M,
    a in M -> insert a (remove a M) =mul= M.
  ...
  Lemma insert_remove_noteq : forall a a' M,
  ...
  Lemma meq_diff_meq : forall M N P, M =mul= N -> M - P =mul= N - P.
...
Section Decidability.
  ...
  Lemma member_dec : forall a M, {a in M}+{~a in M}.
...
End Decidability.
  ...
  Lemma insert_diff : forall a M N, insert a M - N =mul= empty ->
    a in N /\ M - remove a N =mul= empty.
  ...
  Lemma diff_MM_empty : forall M, M - M =mul= empty.

  Proof.
    solve_meq.
  Qed.

  Lemma ins_meq_union : forall M N P a,
  ...
  Lemma mem_memrem : forall M a b, ~ a =A= b -> a in M -> a in (M - {{b}}).
  ...
  Lemma member_notempty : forall M x, x in M -> M <>mul empty.
  ...
  Lemma singleton_notempty : forall x, {{x}} <>mul empty.
  ...
  Lemma union_isempty : forall M N, M + N =mul= empty -> M =mul= empty.
  ...
  Lemma union_notempty : forall M N, M <>mul empty -> M + N <>mul empty.

  Proof.
    compute in *; intros; apply H.
    apply union_isempty with N; trivial.
  Qed.

  Lemma remove_union : forall a L R, a in L -> remove a (L + R) =mul= (remove a L) + R.
  ...
  Lemma meq_remove_elem_right : forall a M L R, M + {{a}} =mul= L + R ->
    a in R -> M =mul= L + (remove a R).

  Proof.
    intros.
    rewrite (union_comm L (remove a R)).
    rewrite <- (remove_union L H0).
    rewrite (union_comm R L).
    rewrite <- H.
    solve_meq.
  Qed.
...
Section MultisetLemmas2.
  ...
  (* Variable gtA_dec : forall a b, {a > b} + {~a > b}. *)

  Lemma double_split : forall L R U D,
  ...
  Proof.
    ...
    case (Compare_dec.le_lt_dec (x/L) (x/U)); intro l_u.
     (* u >= l *)
    assert (x/R >= x/D); [omega | idtac].
    rewrite (Min.min_r (x/R) (x/D)); omega.
     (* l > u *)
    assert (x/R < x/D); [omega | idtac].
    rewrite Minus.not_le_minus_0.
    rewrite (Min.min_l (x/R) (x/D)); omega.
    omega.
  Qed.

  Variables P Q : A -> Prop.
  Variable PQ_dec : forall a, {P a}+{Q a}.

  Definition comp_eqA P := forall a b, a =A= b -> P a -> P b.

  Variable P_comp : comp_eqA P.
  Variable Q_comp : comp_eqA Q.
...
Section List2Multiset.
  ...
  Lemma list2multiset_mult : forall (l: list A) a,
  ...
  Lemma meq_permutation : forall l l',
  ...
  Lemma list2multiset_cons_length : forall l l' a,
  ...
  Lemma list2multiset_insert_nth : forall a l p,
...
Section Multiset2List_Results.
  ...
  Lemma multiset2list_insert : forall a M,
    exists a', exists p, a =A= a' /\ permutation eqA eqA_dec (multiset2list M) (drop_nth (multiset2list (insert a M)) p) /\
  ...
  Lemma multiset2list_cons_length : forall M a,
  ...
  Lemma multiset2list_union_length : forall M N,
    length (multiset2list (M + N)) = (length (multiset2list M) + length (multiset2list N))%nat.
  ...
  Lemma multiset2list_diff_length : forall M N,
  ...
  Lemma multiset2list_meq_length : forall M M',
  ...
  Lemma cons_eq_cons_cons_absurd : forall a a' a'' M,
    {{a}} =mul= {{a'}} + {{a''}} + M -> False.
  ...
  Lemma multiset2list_singleton : forall a,
...
Section MultisetCardinality.
  ...
  Definition card_def (M: Multiset) : {n: nat | length (multiset2list M) = n}.
  ...
  Definition card (M: Multiset) : nat := proj1_sig (card_def M).
  ...
  Lemma multiset2list_card : forall M,
    length (multiset2list M) = card M.
    
  Proof.
    intros.
    unfold card; destruct (card_def M); trivial.
  Qed.

  Lemma card_non_empty : forall M, M <>mul empty -> card M > 0.
  ...
  Lemma singleton_card : forall x, card {{x}} = 1.
  ...
  Lemma pair_card : forall x y, card {{x, y}} = 2.
  
  Proof.
    intros; unfold card, pair, insert.
  ...
  Lemma union_card : forall M N, card (M + N) = (card M + card N)%nat.
  
  Proof.
    intros; unfold card.
    destruct (card_def (M + N)); destruct (card_def M); destruct (card_def N); simpl.
    rewrite <- e; rewrite <- e0; rewrite <- e1.
    apply multiset2list_union_length.
  Qed.

  Lemma diff_card : forall M N, card (M - N) >= card M - card N.
...
Section Pair.
  ...
  Lemma pair_eq : forall a b c d, {{a, b}} =mul= {{c, d}} ->
    (a =A= c /\ b =A= d) \/ (a =A= d /\ b =A= c).
  ...
  Lemma pair_decomp : forall a b X Y,
    {{a, b}} =mul= X + Y ->
  ...
  Proof.
    ...
    rewrite X1c; rewrite X1empty; rewrite ac; solve_meq.
    rewrite Y1d; rewrite Y1empty; rewrite bd; solve_meq.
    right; right; right; split.
    rewrite X1c; rewrite X1empty; rewrite bc; solve_meq.
    rewrite Y1d; rewrite Y1empty; rewrite ad; solve_meq.
    absurd (card {{a, b}} = card (X + Y)).
    repeat (rewrite union_card; try rewrite (card_morph X1c); 
