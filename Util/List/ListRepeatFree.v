...
Lemma repeat_free_last : forall (a : A) l, repeat_free l -> ~In a l -> repeat_free (l ++ a :: nil).
...
Proof.
induction l; simpl; intros. exact H. destruct H. apply repeat_free_last. apply IHl.
exact H0. intro. apply H. apply rev_incl. exact H1.
Qed.

Lemma repeat_free_app_elim : forall l m : list A,
  repeat_free (l ++ m) -> repeat_free l /\ repeat_free m /\ forall x, In x l -> ~In x m.
...
Lemma greatest_repeat_free_prefix_correct : forall l, repeat_free (greatest_repeat_free_prefix l).
...
Lemma greatest_repeat_free_prefix_aux_app : forall m l acc,
  ...
  greatest_repeat_free_prefix_aux acc (l ++ m) = greatest_repeat_free_prefix_aux (rev l ++ acc) m.
...
Lemma greatest_repeat_free_prefix_app : forall m l, repeat_free l ->
  greatest_repeat_free_prefix (l ++ m) = greatest_repeat_free_prefix_aux (rev l) m.
...
Lemma repeat_free_greatest_repeat_free_prefix : forall l, repeat_free l -> greatest_repeat_free_prefix l = l.
...
Lemma greatest_repeat_free_prefix_intro: forall l, l = greatest_repeat_free_prefix l
  \/ exists x, In x (greatest_repeat_free_prefix l) /\ exists p, l = greatest_repeat_free_prefix l ++ x :: p.

Proof.
induction l; intros. simpl. intuition.
assert (repeat_free (greatest_repeat_free_prefix l)). apply greatest_repeat_free_prefix_correct.
...
simpl. deduce (greatest_repeat_free_prefix_aux_elim l (a::nil)). destruct H3. rewrite H3.
simpl. auto.
...
rewrite H3. clear H3. rewrite (greatest_repeat_free_prefix_app (a::x0++x1) H2). simpl.
...
(* ~In a (greatest_repeat_free_prefix l) *)
destruct IHl.
left. symmetry. apply repeat_free_greatest_repeat_free_prefix. simpl. rewrite H0. intuition.
...
assert (a::l = (a::greatest_repeat_free_prefix l)++x::x0). simpl. rewrite <- H1. refl.
rewrite H3. clear H3. rewrite (greatest_repeat_free_prefix_app (x::x0) H0). simpl.
rewrite rev_unit. rewrite rev_involutive.
case (In_dec eq_dec x (rev (greatest_repeat_free_prefix l) ++ a :: nil)); intro. split.
...
Lemma greatest_repeat_free_prefix_intro' : forall l, exists m, l = greatest_repeat_free_prefix l ++ m.
...
Lemma repeat_free_intro: forall l : list A,
  repeat_free l \/ exists m, exists x, exists p, l = m ++ x :: p /\ repeat_free m /\ In x m.

Proof.
intro. deduce (greatest_repeat_free_prefix_intro l).
assert (repeat_free (greatest_repeat_free_prefix l)). apply greatest_repeat_free_prefix_correct. destruct H.
left. rewrite H. exact H0.
right. decomp H. exists (greatest_repeat_free_prefix l). exists x. exists x0. intuition.
Qed.

Lemma repeat_free_intro' : forall l : list A, exists m, exists p, l = m ++ p /\ repeat_free m.
