...
(* $Id: NotSN_IS.v,v 1.1 2007-08-08 09:33:43 blanqui Exp $ *)
...
intro. destruct x. unfold T. simpl. deduce (notSN_succ p). decomp H.
exists (exist P x0 H2). simpl. exact H1.
set (b := exist P a h). deduce (@dep_choice B b T H). destruct H0 as [g].
set (f := fun x => proj1_sig (g x)). exists f. intro. unfold f.
deduce (H0 i). destruct (g i). destruct (g (S i)). unfold T in H1. simpl in H1.
exact H1.
Qed.
