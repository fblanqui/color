...
(** properties *)
  ...
  Section properties.
    ...
    Proof.
      ...
      absurd (cmp x x = Gt). rewrite H. discr. rewrite hyp_symrefl, H. refl.
      absurd (cmp x x = Lt). rewrite H. discr. rewrite hyp_symrefl, H. refl.
    Qed.
    ...
    Proof.
      unfold antisymmetric, greater. intros x y h. rewrite hyp_symrefl, h.
      discr.
    Qed.
...
(*FIXME: already defined in Coq as Compare_dec.nat_compare. *)
...
Proof. unfold is_symrefl. induction x; intros; destruct y; simpl; auto. Qed.

Lemma nat_cmp_trans : is_trans nat_cmp.

Proof.
  unfold is_trans. induction x; induction y; simpl; intros; destruct z; simpl;
    subst; auto; try discr.
  trans (nat_cmp y z). 2: hyp. apply (IHx y z (nat_cmp y z)); auto.
Qed.
