...
(*FIXME: moved to VecOrd. eq_vec is nothing but Vreln. *)
...
Section eq_vec.
  ...
  Proof. unfold eq_vec. intros. apply Vforall2n_intro. refl. Qed.
  ...
  Proof. unfold eq_vec, Vforall2n. intros. simpl. tauto. Qed.
...
(** morphisms *)
  ...
  Proof. intros. apply Vforall2n_nth with (R:=eqA). hyp. Qed.

  Lemma Veq_vec_nth : forall n (v v' : vec n),
    (forall i (ip : i < n), Vnth v ip =A= Vnth v' ip) -> v =v v'.

  Proof. apply Vforall2n_intro. Qed.
  ...
  Section Vforall.
    ...
    Proof. intros. unfold Vforall2n. apply Vforall2n_aux_mor; hyp. Qed.
...
(** (Vfold_left f) is a morphism is f is a morphism *)
  ...
  Section Vfold_left.
    ...
    Lemma Vfold_left_mor : forall b b', b =B= b' -> forall n (v v': vec n),
      v =v v' -> Vfold_left f b v =B= Vfold_left f b' v'.

    Proof.
      induction v; simpl; intros. VOtac. simpl. hyp. revert H0. VSntac v'.
      rewrite eq_vec_cons. intuition. simpl. apply f_mor; try hyp.
      apply IHv; hyp.
    Qed.
...
(** (Vmap2 f) is a morphism is f is a morphism *)
  ...
  Section Vmap2.
    ...
    Proof.
      induction va; simpl; intros. refl. revert H. VSntac va'.
      revert H0. VSntac vb. VSntac vb'. rewrite !eq_vec_cons. intuition.
    Qed.
