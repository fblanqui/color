...
Proof.
intro. apply term_ind_forall; intros. refl. simpl. case (pi f). intros [k h].
rewrite Vnth_map. apply Vforall_nth. hyp.
...
Proof.
intro. unfold transitive, proj_ord. intros. eapply H. apply H0. assumption.
Qed.
...
repeat rewrite proj_sub. apply H. assumption.
Qed.
...
simpl. assumption. simpl. case_eq (pi f). destruct s as [k h].
repeat rewrite Vnth_cast. repeat rewrite Vnth_app.
case (le_gt_dec i k); intro. case (eq_nat_dec i k); intro.
subst. repeat rewrite Vnth_head; try omega. apply IHc. hyp.
...
repeat (rewrite Vnth_head; [idtac|omega]). hyp.
