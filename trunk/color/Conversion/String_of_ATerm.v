...
Proof.
induction s; intros. refl. simpl term_of_string. rewrite string_of_term_fun1.
rewrite IHs. refl.
Qed.
...
elim s. refl. intros. simpl term_of_string. rewrite H. rewrite sub_fun1. refl.
unfold SContext.fill. simpl lft. simpl rgt. intros. simpl term_of_string.
simpl cont_of_string. rewrite fill_cont1. apply args_eq. apply Vcast_eq_intro.
...
Focus 2. ded (h2 _ _ H0). cut (maxvar r' <= maxvar l'). intro. omega.
...
change (In (srule_of_rule (mkRule x x0)) (srs_of_trs R)). apply in_map. hyp.
Qed.
...
apply H1 with (string_of_term y). 2: refl. subst x. apply sred_mod_of_red_mod.
hyp.
Qed.
