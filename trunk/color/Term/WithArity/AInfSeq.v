...
apply plus_lt_reg_l with (p := fst (F0 (F1 i))). rewrite le_plus_minus_r; auto.
rewrite le_plus_minus_r; auto. apply (@le_trans _ _ _ H1 (@lt_le_weak _ _ H2)).
...
generalize (HP j). intro HPj. destruct (hyp1 j) as [_ ERj]. destruct ERj; auto.
...
(***********************************************************************)
(** Building an infinite sequence starting from an infinite sequence
modulo. *)
...
Lemma ISOfISMod_rec_spec : forall i,
  E (fst (proj1_sig (ISOfISMod_rec i))) (fst (proj1_sig (ISOfISMod_rec (S i)))).
...
(***********************************************************************)
(** Building an infinite sequence of transitive closure *)
...
generalize (Hproj j). intros HT. cut (E # (f j) (g j)).
intros HT0. destruct (rtc_split HT0) as [| HT1]; auto.
destruct (le_not_lt _ _ (HT (conj le_Sij HT1))). hyp. apply (proj1 (hyp1 j)).
...
apply (@TrsR _ (f k)). apply IHk; try omega. rewrite (eq_fgi i k); try omega.
apply (proj2 (hyp1 k)).
...
exists f0; exists g0. split. intro i. simpl. unfold f0, g0. split. apply HEfg.
