...
a simple overGraph on DP based on the equality of head symbols
*)
...
Lemma in_rule_dec : forall (a : rule) L, {In a L} + {~In a L}.
...
Lemma int_red_preserv_hd : forall t1 t2, int_red E t1 t2 ->
...
Lemma int_red_rtc_preserv_hd : forall t1 t2, (int_red E) # t1 t2 ->
...
Definition int_lhs_rules_only Sig L := forall l r, In (mkRule l r) L ->
  exists f, exists v, l = @Fun (dup_sig Sig) (int_symb Sig f) v.

Variable int_hyp : int_lhs_rules_only E. 

Lemma dup_int_rules_int_red : forall f v t,
  red E (@Fun (dup_sig Sig) (hd_symb _ f) v) t -> 
...
Lemma dup_int_rules_int_red_rtc_aux : forall u t, red E # u t ->
  forall f v, u = @Fun (dup_sig Sig) (hd_symb _ f) v -> 
...
Definition hd_rhs_rules_only Sig L := forall l r, In (mkRule l r) L ->
  exists f, exists v, r = @Fun (dup_sig Sig) (hd_symb Sig f) v.
...
apply (@int_red_hd_rules_graph_incl_hde _ E R
  (mkRule lhs (@Fun (dup_sig Sig) (hd_symb Sig x) x2)) y).
...
rewrite app_fun.
change ((int_red E #)
  (@Fun (dup_sig Sig) (hd_symb Sig x) (Vmap (ASubstitution.app x1) x2))
...
Lemma In_cons : forall (A : Set) (x a : A) l, In x (a::l) <-> a=x \/ In x l. 
